...
(function () {
  ...
  function sync() {
    if (hid) hid.value = sw.checked ? 1 : 0;
...
(function () {
  ...
  function calculateAdjustment() {
    ...
    if (adjustmentInfo) adjustmentInfo.style.display = "block";

    const langIncreaseStock =
      window.outletProductLang?.type_increase || "Increase";
    const langDecreaseStock =
      window.outletProductLang?.type_decrease || "Decrease";

    if (difference > 0) {
      if (adjustmentInfo)
        adjustmentInfo.className = "alert alert-success py-1 px-2";
      if (adjustmentType) adjustmentType.textContent = langIncreaseStock;
      if (adjustmentAmount)
        adjustmentAmount.textContent = "+" + difference + " pcs";
    } else {
      if (adjustmentInfo)
        adjustmentInfo.className = "alert alert-danger py-1 px-2";
      if (adjustmentType) adjustmentType.textContent = langDecreaseStock;
      if (adjustmentAmount)
...
(function () {
  ...
  // MENGAMBIL PREFIX ROLE DARI WINDOW ATAU URL
  // Cek apakah url saat ini mengandung '/manager/' atau '/supervisor/'
  const currentPath = window.location.pathname;
  let staffPrefix = "manager"; // Default
  if (currentPath.includes("/supervisor/")) {
    staffPrefix = "supervisor";
  }

  // MENYUSUN URL DINAMIS UNTUK STAFF
  const SAVE_URL = `/employee/${staffPrefix}/products/recipe/save`;
  const LOAD_URL = `/employee/${staffPrefix}/products/recipe/load`;
  const INGREDIENTS_URL = `/employee/${staffPrefix}/products/recipe/ingredients`;
  ...
  function loadRecipeData() {
    ...
    // Fetch data ingredients (Staff hanya mengakses data dari 1 outlet)
  ...
  btnManageProductRecipe?.addEventListener("click", function (e) {
    ...
    // currentPartnerId tidak perlu dilempar lagi karena Staff hanya mengelola 1 outlet miliknya.
