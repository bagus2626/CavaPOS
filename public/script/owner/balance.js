window.initBalanceTab = function (accountId) {
  ...
  $(document).ready(function () {
    ...
    $("#popup-balance-filter-options").on(
      "change",
      ".balance-filter-checkbox, .filter-checkbox",
      function () {
        ...
        if (this.checked) {
          ...
          activeBalanceFilters[group] = activeBalanceFilters[group].filter(
            (item) => item !== value
          );
        }
    ...
    $(".search-balance-type-select").on("click", function (e) {
      ...
      $("#global-balance-search-input").attr(
        "placeholder",
        `Search ${label}...`
      );
    });
    ...
    $(document).on(
      "click",
      "#xendit-balance-pagination .page-link",
      function (e) {
        e.preventDefault();
  ...
  function displayDataBalances(filter_data = {}, pagination_params = {}) {
    ...
    $.ajax({
      url: `/owner/user-owner/xen_platform/accounts/filter/balance`,
      method: "GET",
      data: requestData,
      success: async function (response) {
        ...
        const isEmptyTable =
          $rows.length === 1 && $rows.first().find("td[colspan]").length > 0;

        if (!isEmptyTable && $rows.length > 0) {
          const currentLimit = parseInt($("#balance-filter-limit").val()) || 10;

          // Gunakan currentLimit, bukan pageBalanceLimit yang statis
          const startNumber = (currentBalancePage - 1) * currentLimit + 1;
      ...
      error: function (err) {
        ...
        $(`#show-data-balance`).html(
          '<div class="text-center text-danger py-2">Gagal memuat data Balance. Silakan coba lagi.</div>'
        );
      },
    });
  }
