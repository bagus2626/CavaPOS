      ...
      <form action="{{ route('partner.store.tables.update', $table->id) }}" method="POST" enctype="multipart/form-data" id="tableEditForm">
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            <label for="table_no" class="form-label">{{ __('messages.partner.outlet.table_management.tables.table_no') }}</label>
          ...
          {{-- Status --}}
          <div class="col-md-6 mb-3">
            <label for="status" class="form-label">{{ __('messages.partner.outlet.table_management.tables.status') }}</label>
            <select name="status" id="status"
                    class="form-control select2 @error('status') is-invalid @enderror" required>
              <option value="">{{ __('messages.partner.outlet.table_management.tables.choose_status') }}</option>
              <option value="available" {{ old('status', $table->status) == 'available' ? 'selected' : '' }}>{{ __('messages.partner.outlet.table_management.tables.available') }}</option>
              <option value="occupied"  {{ old('status', $table->status) == 'occupied'  ? 'selected' : '' }}>{{ __('messages.partner.outlet.table_management.tables.occupied') }}</option>
              <option value="reserved"  {{ old('status', $table->status) == 'reserved'  ? 'selected' : '' }}>{{ __('messages.partner.outlet.table_management.tables.reserved') }}</option>
            </select>
            @error('status')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            <label for="table_class" class="form-label">{{ __('messages.partner.outlet.table_management.tables.class_type') }}</label>
            <input list="table_class_list"
                   ...
                   placeholder="{{ __('messages.partner.outlet.table_management.tables.placeholder_1') }}" required>
          ...
          <div class="col-md-6 mb-3">
            ...
            <small class="text-muted d-block">{{ __('messages.partner.outlet.table_management.tables.new_upload') }} <b>{{ __('messages.partner.outlet.table_management.tables.replace') }}</b> {{ __('messages.partner.outlet.table_management.tables.old_picture') }}</small>
            @error('images')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            @error('images.*')
            ...
            @if($table->images && count($table->images) > 0)
              <div class="mt-3">
                <label class="form-label">{{ __('messages.partner.outlet.table_management.tables.current_images') }}</label>
        ...
        {{-- Description --}}
        <div class="mb-3">
          <label for="description" class="form-label">{{ __('messages.partner.outlet.table_management.tables.description') }}</label>
