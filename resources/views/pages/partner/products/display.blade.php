    ...
    <tbody>
      @foreach ($products as $index => $product)
        <tr data-category="{{ $product->category_id }}">
          ...
          <td class="col-desc"><div class="clamp-2">{{ $product->description ?? '—' }}</div></td>
            ...
            @if(!empty($product->pictures) && is_array($product->pictures))
              <div class="thumb-list">
                @foreach($product->pictures as $picture)
                  ...
                  <a href="{{ $src }}" target="_blank" rel="noopener" class="thumb-item" title="{{ $picture['filename'] ?? 'Product Image' }}">
                    <img src="{{ $src }}" alt="{{ $picture['filename'] ?? ($product->name . ' image') }}" class="thumb-img-80">
                  </a>
                @endforeach
              </div>
            @else
...
/* Thumbnails */
.thumb-list{ display:flex; flex-wrap:wrap; gap:.35rem; }
.thumb-item{ display:block; }
.thumb-img-80{
  width:80px; height:80px; object-fit:cover;
  border-radius:10px; border:0; box-shadow: 0 6px 20px rgba(0,0,0,.08);
...
.col-images{ min-width: 160px; }
.col-options{ min-width: 180px; }
