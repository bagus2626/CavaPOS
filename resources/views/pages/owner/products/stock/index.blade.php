                        ...
                        <div class="search-filter-group">
                            ...
                            <div class="select-wrapper" style="min-width: 200px;">
                                <form action="{{ route('owner.user-owner.stocks.index') }}" method="GET" id="locationFilterForm">
                                    <select id="locationFilter" name="filter_location" class="form-control-modern" onchange="this.form.submit()">
...
<style>
    ...
    .mobile-category-dropdown { margin-top: 12px; }
    .select-wrapper-mobile { position: relative; width: 100%; }
    .form-control-mobile {
        width: 100%; padding: 12px 40px 12px 16px; border: 1px solid #e0e0e0;
        border-radius: 8px; font-size: 14px; background-color: #fff;
        appearance: none; -webkit-appearance: none; -moz-appearance: none; cursor: pointer;
    }
    .form-control-mobile:focus { outline: none; border-color: #ae1504; }
    .select-arrow-mobile {
        position: absolute; right: 12px; top: 50%; transform: translateY(-50%);
        pointer-events: none; color: #666; font-size: 20px;
    }
</style>
...
@push('scripts')
    ...
    <script>
        ...
        function debouncedStockSearch(el, delay = 400) {
            ...
            el._debounceTimer = setTimeout(() => { window.__applyStockFilter?.(); }, delay);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateMobileSubtitle() {
                if (mobileStockSubtitle) {
                    mobileStockSubtitle.textContent = `${filtered.length} {{ __('messages.owner.products.stocks.stock_list') }}`;
                }
            }
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="text-center">
                            <div class="table-actions">
                                <button onclick="deleteStock(${s.id})" class="btn-table-action delete" title="{{ __('messages.owner.products.stocks.delete') }}">
                ...
                items.forEach(s => {
                    ...
                    card.innerHTML = `
                        ...
                        <div class="stock-card__bottom">
                            ...
                            <span class="stock-chip"><span class="material-symbols-outlined">payments</span><span>${escapeHtml(String(s.last_price_per_unit ?? ''))}</span></span>
