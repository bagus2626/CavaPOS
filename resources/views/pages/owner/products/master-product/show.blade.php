...
@section('content')
    @php
        ...
        // kategori (opsional)
        $catName =
            optional($prod->category)->category_name ?? __('messages.owner.products.master_products.uncategorized');
                ...
                @if (!empty($prod->pictures) && is_array($prod->pictures) && count($prod->pictures) > 1)
                    <div class="detail-gallery">
                        @foreach ($prod->pictures as $p)
                            @php
                                $src = !empty($p['path'])
                                    ? (Str::startsWith($p['path'], ['http://', 'https://'])
                                        ? $p['path']
                                        : asset($p['path']))
                                    : null;
                            @endphp
                            @if ($src)
                                <a href="{{ $src }}" target="_blank" rel="noopener" class="gallery-item">
                                    <img src="{{ $src }}"
                                        alt="{{ $p['filename'] ?? __('messages.owner.products.master_products.gallery_image_alt') }}">
                                </a>
                            @endif
                        @endforeach
                    </div>
            ...
            <div class="modern-card">
                <div class="card-body-modern">
                    ...
                    <div class="section-header">
                        ...
                        <h3 class="section-title">{{ __('messages.owner.products.master_products.product_information') }}
                        </h3>
                    </div>
                    ...
                    <div class="action-buttons-group">
                        <a href="{{ route('owner.user-owner.master-products.edit', $prod->id) }}"
                            class="btn-action btn-action-edit">
                            <span class="material-symbols-outlined">edit</span>
                            {{ __('messages.owner.products.master_products.edit') }}
                        </a>

                        <form action="{{ route('owner.user-owner.master-products.destroy', $prod->id) }}" method="POST"
                                    ...
                                    <table class="data-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-end">
                                                    {{ __('messages.owner.products.master_products.price') }}</th>
