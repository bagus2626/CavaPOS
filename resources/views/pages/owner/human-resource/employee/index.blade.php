...
<section class="content">
    <div class="container-fluid">
        <a href="{{ route('owner.user-owner.employees.create') }}" class="btn btn-primary mb-3">Add Employee</a>
        <div class="mb-3">
            <button class="btn btn-outline-secondary btn-sm filter-btn rounded-pill active" data-category="all">All</button>
            @foreach($partners as $partner)
                <button class="btn btn-outline-secondary btn-sm filter-btn rounded-pill" data-category="{{ $partner->id }}">
                    {{ $partner->name }}
                </button>
