                    ...
                    <div class="bg-white/95 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden relative">
                        ...
                        <div class="px-6 sm:px-8 pt-5 sm:pt-8 pb-3 sm:pb-6 text-center relative">
                            <h1 class="text-xl sm:text-2xl font-bold text-[#991b1b]">
                                {{ __('messages.owner.auth.login.title') }}
                            </h1>
                            <p class="mt-2 sm:mt-3 text-xs sm:text-sm text-gray-600">
                                {!! __('messages.owner.auth.login.subtitle') !!}
                            </p>
                        </div>
                        ...
                        <form method="POST" action="{{ route('owner.login.attempt') }}" class="px-6 sm:px-8 pb-5 sm:pb-7 relative">
                            ...
                            <div class="mt-2">
                                <label for="email" class="block text-xs sm:text-sm font-semibold text-gray-700 mb-1.5 sm:mb-2">
                                    {{ __('messages.owner.auth.login.email_label') }}
                                </label>
                                <div class="relative">
                                    ...
                                    <input id="email"
                                           ...
                                           placeholder="{{ __('messages.owner.auth.login.email_placeholder') }}"
                            ...
                            <div class="mt-3 sm:mt-4">
                                <div class="flex items-center justify-between mb-1.5 sm:mb-2">
                                    <label for="password" class="block text-xs sm:text-sm font-semibold text-gray-700">
                                        {{ __('messages.owner.auth.login.password_label') }}
                                    </label>
                                    @if (Route::has('owner.password.request'))
                                        <a href="{{ route('owner.password.request') }}"
                                           class="text-xs sm:text-sm text-[#dc2626] hover:text-[#991b1b] transition-colors font-semibold">
                                            {{ __('messages.owner.auth.login.forgot_password') }}
                                        </a>
                                    @endif
                                </div>
                                ...
                                <div class="relative">
                                    ...
                                    <input id="password"
                                           ...
                                           placeholder="{{ __('messages.owner.auth.login.password_placeholder') }}"
                            ...
                            <div class="block mt-3 sm:mt-4">
                                <label for="remember" class="inline-flex items-center select-none cursor-pointer">
                                    ...
                                    <input id="remember" type="checkbox" value="1"
                                           ...
                                           name="remember">
                                    <span class="ms-2 text-xs sm:text-sm text-gray-700 font-medium">
                                        {{ __('messages.owner.auth.login.remember_me') }}
                                    </span>
                                </label>
                            </div>
                            ...
                            <div class="mt-4 sm:mt-5">
                                <button type="submit"
                                    ...
                                    {{ __('messages.owner.auth.login.submit_button') }}
                                </button>
                            </div>
                            ...
                            <div class="mt-4 sm:mt-5 flex items-center gap-2 sm:gap-3">
                                ...
                                <span class="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wider font-semibold">
                                    {{ __('messages.owner.auth.login.divider_text') }}
                                </span>
                                <div class="h-px bg-gradient-to-r from-transparent via-red-200 to-transparent flex-1"></div>
                            </div>
                            ...
                            <a href="{{ route('owner.google.redirect') }}"
                               class="mt-3 sm:mt-4 inline-flex w-full py-3 sm:py-3.5 items-center justify-center gap-2 sm:gap-3 rounded-lg sm:rounded-xl 
                                ...
                                <span class="text-xs sm:text-sm font-semibold">
                                    {{ __('messages.owner.auth.login.google_button') }}
                                </span>
                            </a>
                        </form>
