...
<body class="bg-gray-50">
    @php
        $data = session('partner_suspended_data', []);
            ...
            <div class="bg-white rounded-lg shadow-lg p-8 text-center">
                ...
                <div class="text-gray-600 mb-6 break-words" style="word-break: break-word; overflow-wrap: break-word;">
                    @if($suspendedBy == 'admin')
                        ...
                        @else
                            <p>Outlet <strong>{{ $partnerName }}</strong> saat ini ditangguhkan sementara oleh administrator sistem.</p>
                        @endif
                    @else
                        <p>Outlet <strong>{{ $partnerName }}</strong> saat ini dinonaktifkan sementara oleh owner <strong>{{ $ownerName }}</strong>.</p>
                    @endif
                </div>
