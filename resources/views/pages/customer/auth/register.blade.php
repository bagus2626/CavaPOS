...
<div class="w-full max-w-md bg-white shadow-lg rounded-lg p-6 md:p-8 my-10">
    ...
    <form method="POST" action="{{ route('customer.register.submit', ['partner_slug' => $partner_slug, 'table_code' => $table_code]) }}" class="space-y-4">
        ...
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700">{{ __('messages.customer.register.full_name') }}</label>
        ...
        <div>
            <label for="phone" class="block text-sm font-medium text-gray-700">{{ __('messages.customer.register.phone_number') }}</label>
        ...
        <div>
            <label for="password" class="block text-sm font-medium text-gray-700">{{ __('messages.customer.register.password') }}</label>
        ...
        <div>
            <label for="password_confirmation" class="block text-sm font-medium text-gray-700">{{ __('messages.customer.register.password_confirmation') }}</label>
        ...
        <div>
            <button type="submit"
                ...
                {{ __('messages.customer.register.register') }}
            </button>
        </div>
    </form>
    ...
    <div class="flex items-center my-6">
        ...
        <span class="mx-2 text-gray-400 text-sm">{{ __('messages.customer.register.or') }}</span>
        <hr class="flex-grow border-gray-300">
    </div>
    ...
    <div class="space-y-3">
        <a href="{{ route('customer.google.redirect', ['provider' => 'google', 'partner_slug' => $partner_slug, 'table_code' => $table_code]) }}"
           class="w-full inline-flex justify-center items-center border border-gray-300 rounded-md py-2 px-4 mb-2 hover:bg-gray-100 transition">
            ...
            {{ __('messages.customer.register.register_with_google') }}
        </a>
        ...
        <a href="{{ route('customer.login', ['partner_slug' => $partner_slug, 'table_code' => $table_code]) }}"
           class="w-full bg-gray-700 hover:bg-gray-800 text-white font-semibold py-2 px-4 rounded-md transition text-center block">
            {{ __('messages.customer.register.have_an_account_yet_login') }}
        </a>
    </div>
</div>
@endsection
