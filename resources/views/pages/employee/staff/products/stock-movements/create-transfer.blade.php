...
@section('title', __('messages.owner.products.stocks.movements_transfer.title') ?? 'Stock Transfer')
@section('page_title', __('messages.owner.products.stocks.movements_transfer.page_title') ?? 'Create Transfer')
...
@section('content')
        ...
        <div class="container-modern">
            <div class="page-header">
                <div class="header-content">
                    <h1 class="page-title">{{ __('messages.owner.products.stocks.movements_transfer.page_title') ?? 'Stock Transfer' }}</h1>
                    <p class="page-subtitle">{{ __('messages.owner.products.stocks.movements_transfer.subtitle') ?? 'Move stock to another location' }}</p>
                </div>
                ...
                <form action="{{ route('employee.' . $staffRoutePrefix . '.stocks.movements.store') }}" method="POST" id="stockMovementForm">
                    ...
                    <div class="card-body-modern">
                        <div class="section-header">
                            ...
                            <h3 class="section-title">{{ __('messages.owner.products.stocks.movements_transfer.card_transaction_title') ?? 'Transaction Details' }}</h3>
                        </div>
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="form-group-modern">
                                    <label class="form-label-modern">
                                        {{ __('messages.owner.products.stocks.movements_transfer.location_to_label') ?? 'Location To' }}
                                        <span class="text-danger">*</span>
                                    </label>
                                    <div class="select-wrapper">
                                        <select name="location_to" id="location_to" class="form-control-modern @error('location_to') is-invalid @enderror" required>
                                            <option value="_owner" selected>
                                                    {{ __('messages.owner.products.stocks.movements_transfer.location_owner_option') ?? 'Gudang Pusat (Owner)' }}
                                            </option>
                                        </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group-modern">
                                    <label class="form-label-modern">
                                        {{ __('messages.owner.products.stocks.movements_transfer.notes_label') ?? 'Notes' }}
                                    </label>
                                    <input type="text" name="notes" id="notes"
                                        class="form-control-modern @error('notes') is-invalid @enderror"
                                        placeholder="{{ __('messages.owner.products.stocks.movements_transfer.notes_placeholder') ?? 'Transfer notes' }}">
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="section-header">
                            ...
                            <h3 class="section-title">{{ __('messages.owner.products.stocks.movements_transfer.card_items_title') ?? 'Items to Transfer' }}</h3>
                        </div>
                        ...
                        <div id="item-repeater-container">
                            <div class="row repeater-item g-3 mb-3">
                                <div class="col-md-5">
                                    <div class="form-group-modern">
                                        <label class="form-label-modern">
                                            {{ __('messages.owner.products.stocks.movements_transfer.item_stock_label') ?? 'Stock Item' }}
                                            <span class="text-danger">*</span>
                                        </label>
                                        <div class="select-wrapper">
                                            <select name="items[0][stock_id]" class="form-control-modern stock-select" required>
                                                <option value="">
                                                    {{ __('messages.owner.products.stocks.movements_transfer.item_stock_placeholder') ?? 'Select Item' }}
                                                </option>
                                ...
                                <div class="col-md-3">
                                    <div class="form-group-modern">
                                        <label class="form-label-modern">
                                            {{ __('messages.owner.products.stocks.movements_transfer.quantity_label') ?? 'Quantity' }}
                                            <span class="text-danger">*</span>
                                        </label>
                                        <input type="number" name="items[0][quantity]" class="form-control-modern quantity-input"
                                            step="0.01" min="0.01"
                                            placeholder="{{ __('messages.owner.products.stocks.movements_transfer.quantity_placeholder') ?? '0.00' }}"
                                            required>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group-modern">
                                        <label class="form-label-modern">
                                            {{ __('messages.owner.products.stocks.movements_transfer.unit_label') ?? 'Unit' }}
                                            <span class="text-danger">*</span>
                                        </label>
                                        <div class="select-wrapper">
                                            <select name="items[0][unit_id]" class="form-control-modern unit-select" required>
                                                <option value="">
                                                    {{ __('messages.owner.products.stocks.movements_transfer.unit_placeholder_no_stock') ?? 'Select Unit' }}
                                                </option>
                                            </select>
                                            <span class="material-symbols-outlined select-arrow">expand_more</span>
                                        </div>
                        ...
                        <button type="button" id="btn-add-item" class="btn-modern btn-sm-modern btn-secondary-modern" disabled>
                            ...
                            {{ __('messages.owner.products.stocks.movements_transfer.add_item_button') ?? 'Add Item' }}
                        </button>
                    </div>
                    ...
                    <div class="card-footer-modern">
                        ...
                        <button type="submit" class="btn-submit-modern" id="btn-submit" disabled>
                            {{ __('messages.owner.products.stocks.movements_transfer.submit_button') ?? 'Submit Transfer' }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (!container || !template || !btnAddItem || !locationTo) return;
            ...
            function updateRowInfo(stockSelectElement) {
                ...
                unitSelect.innerHTML =
                    `<option value="">{{ __('messages.owner.products.stocks.movements_transfer.unit_placeholder') ?? 'Select Unit' }}</option>`;
                ...
                if (currentQty && infoBox) {
                    infoBox.textContent =
                        '{{ __('messages.owner.products.stocks.movements_transfer.current_stock_prefix') ?? 'Current Stock: ' }} ' +
            ...
            function isFormValid() {
                if (!locationTo.value) return false;
            ...
            function updateButtons() {
                ...
                btnAddItem.disabled = false; // Add item tidak bergantung pada validasi form
            }
            ...
            form.addEventListener('submit', function (e) {
                ...
                Swal.fire({
                    title: '{{ __('messages.owner.products.stocks.movements_transfer.confirm_title') ?? 'Confirm Transfer?' }}',
                    text: '{{ __('messages.owner.products.stocks.movements_transfer.confirm_text') ?? 'Please verify the stock transfer details before saving.' }}',
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#8c1000',
                    cancelButtonColor: '#6b7280',
                    confirmButtonText: '{{ __('messages.owner.products.stocks.movements_transfer.confirm_button') ?? 'Yes, Transfer' }}',
                    cancelButtonText: '{{ __('messages.owner.products.stocks.movements_transfer.cancel_button') ?? 'Cancel' }}',
                    reverseButtons: true
                }).then((result) => {
