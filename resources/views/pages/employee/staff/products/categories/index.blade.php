...
@section('content')
    ...
    <div class="modern-container">
        <div class="container-modern">
                ...
                <div class="card-body-modern" style="padding: var(--spacing-lg) var(--spacing-xl);">
                    <div class="table-controls">
                        ...
                        <div style="display: flex; gap: var(--spacing-sm);">
                            ...
                            <a href="{{ route("employee.{$empRole}.categories.create") }}"
            ...
            @include('pages.employee.staff.products.categories.display')

        </div>
    </div>

    @include('pages.employee.staff.products.categories.category-order-modal')
    ...
    <script>
        $(function () {
            ...
            function initSortable() {
                $("#sortableCategoryList").sortable({ placeholder: "ui-state-highlight", axis: "y" });
                $("#sortableCategoryList").disableSelection();
            }

            $('#orderModal').on('shown.bs.modal', function () { initSortable(); });
            ...
            $("#saveOrderBtn").on('click', function () {
                ...
                $("#sortableCategoryList li").each(function (index, el) {
                    orderedIDs.push({ id: $(el).data("id"), order: index + 1 });
                });

                $.ajax({
                    url: "{{ route("employee.{$empRole}.categories.reorder") }}",
                    method: "POST",
                    data: { _token: "{{ csrf_token() }}", orders: orderedIDs },
                    success: function () {
                        Swal.fire({
                            icon: 'success',
                    ...
                    error: function () {
                        Swal.fire('{{ __('messages.owner.products.categories.error_title') }}',
                            '{{ __('messages.owner.products.categories.order_updated_error') }}', 'error');
                    }
                });
            });
        });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function emptyStateHtml() {
                return `<div class="table-empty-state" style="padding: 20px;">
            ...
            function renderMobile(list) {
                ...
                if (!list.length) { mobileList.innerHTML = emptyStateHtml(); return; }
                ...
                list.forEach(c => {
                    ...
                    const editUrl   = `/employee/${empRole}/categories/${c.id}/edit`;
                    const deleteUrl = `/employee/${empRole}/categories/${c.id}`;
                    ...
                    card.innerHTML = `
                        ...
                        <div class="category-card__bottom">
                            ...
                            <div class="category-card__actions">
                                ...
                                <form action="${deleteUrl}" method="POST" class="js-delete-form-mobile"
                                    data-name="${escapeHtml(c.category_name)}" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
