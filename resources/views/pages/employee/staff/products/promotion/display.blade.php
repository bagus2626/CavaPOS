...
<div class="modern-card">
                    ...
                    <tr class="table-row">
                        ...
                        <td>
                            <div class="text-secondary" style="font-size:0.875rem;">
                                ...
                                @elseif($promotion->start_date)
                                    {{ __('messages.owner.products.promotions.start') }} {{ $promotion->start_date->translatedFormat('d F Y') }}
                                @elseif($promotion->end_date)
                                    {{ __('messages.owner.products.promotions.until') }} {{ $promotion->end_date->translatedFormat('d F Y') }}
                                @else
                        ...
                        <td class="text-center">
                            <div class="table-actions">
                                <a href="{{ route("employee.{$empRole}.promotions.show", $promotion->id) }}"
                                    class="btn-table-action view" title="{{ __('messages.owner.products.promotions.detail') }}">
                                    <span class="material-symbols-outlined">visibility</span>
                                </a>
                                <a href="{{ route("employee.{$empRole}.promotions.edit", $promotion->id) }}"
                                    class="btn-table-action edit" title="{{ __('messages.owner.products.promotions.edit') }}">
                                    <span class="material-symbols-outlined">edit</span>
                                </a>
                                <form action="{{ route("employee.{$empRole}.promotions.destroy", $promotion->id) }}"
                    ...
                    <tr>
                        <td colspan="9" class="text-center">
                            <div class="table-empty-state">
                                ...
                                <h4>{{ __('messages.owner.products.promotions.no_promotions') }}</h4>
                                <p>{{ __('messages.owner.products.promotions.add_first_promotion') }}</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
        ...
        <div class="mobile-header-section">
            <div class="mobile-header-card">
                ...
                <div class="mobile-search-wrapper">
                    <form method="GET" action="{{ route("employee.{$empRole}.promotions.index") }}">
            ...
            @forelse ($promotions as $promotion)
                ...
                <div class="employee-card-wrapper">
                    <div class="swipe-actions">
                        <a href="{{ route("employee.{$empRole}.promotions.edit", $promotion->id) }}" class="swipe-action edit">
                            <span class="material-symbols-outlined">edit</span>
                        </a>
                        <button type="button"
                    ...
                    <a href="{{ route("employee.{$empRole}.promotions.show", $promotion->id) }}" class="employee-card-link">
                            ...
                            <div class="employee-card__left">
                                ...
                                <div class="employee-card__info">
                                    ...
                                    <div style="margin-top: 6px;">
                                        <span class="badge-modern {{ $badgeClass }}" style="font-size:11px; padding:4px 10px;">{{ $label }}</span>
                                    </div>
                                </div>
                            </div>
    ...
    @if ($promotions->hasPages())
        <div class="table-pagination">{{ $promotions->links() }}</div>
    @endif
</div>

{{-- Floating Add Button Mobile --}}
<a href="{{ route("employee.{$empRole}.promotions.create") }}" class="btn-add-employee-mobile">
    ...
    <div class="filter-modal-content">
        ...
        <div class="filter-modal-body">
            <div class="modal-filter-pills">
                <a href="{{ route("employee.{$empRole}.promotions.index", array_filter(['q' => request('q')])) }}"
                ...
                <div class="filter-divider"><span>Promotion Types</span></div>

                <a href="{{ route("employee.{$empRole}.promotions.index", array_filter(['q' => request('q'), 'type' => 'percentage'])) }}"
                    class="modal-pill {{ request('type') === 'percentage' ? 'active' : '' }}" onclick="closeMobileFilter()">
                    <div class="pill-left">
                ...
                <a href="{{ route("employee.{$empRole}.promotions.index", array_filter(['q' => request('q'), 'type' => 'amount'])) }}"
                    class="modal-pill {{ request('type') === 'amount' ? 'active' : '' }}" onclick="closeMobileFilter()">
                    <div class="pill-left">
        ...
        <div class="filter-modal-footer">
            <button type="button" class="btn-clear-filter"
                onclick="window.location.href='{{ route("employee.{$empRole}.promotions.index", array_filter(['q' => request('q')])) }}'">
...
<script>
    function toggleMobileFilter() {
        document.getElementById('mobileFilterModal').classList.add('show');
    ...
    function searchFilter(input, delay) {
        ...
        searchTimeout = setTimeout(() => input.form.submit(), delay);
    }
    ...
    function deletePromotion(promotionId, promotionName) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = `/employee/{{ $empRole }}/promotions/${promotionId}`;
